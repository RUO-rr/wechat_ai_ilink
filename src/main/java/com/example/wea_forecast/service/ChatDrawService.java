...
public class ChatDrawService {
    ...
    public String draw(String userId, String userMessage) {
        // 1. 发送等待提示
        try {
            ilinkService.sendText(userId, "正在生成您的图片，请稍候...");
        } catch (Exception e) {
        ...
        // 5. 发送图片
        try {
            ilinkService.sendImage(userId, imageBytes, "generated.png", null);
        } catch (Exception e) {
    ...
    private String optimizePrompt(String userId, String userMessage) {
        ...
        // 取历史快照 + 追加优化指令
        List<Map<String, String>> messages = history.getSnapshot(userId);
        ...
        try {
            ...
            log.info("优化提示词"+root.get("choices").get(0).get("message").get("content").asText().trim());
    ...
    private byte[] callDrawApi(String prompt) throws Exception {
        ...
        log.info("🚀 请求生图 API 的完整地址: {}", apiUrl);
        ...
        log.info("🔍 生图 API 原始响应: {}", responseBody);
    ...
    private void trySendError(String userId, String message) {
        try {
            ilinkService.sendText(userId, "【错误】" + message);
        } catch (Exception ex) {
