...
public class ChatTTSService {
    ...
    public ChatTTSService(RestTemplate restTemplate) {
    ...
    public byte[] synthesize(String userId, String text) {
        ...
        // 打印完整请求体 JSON，方便排查问题
        try {
            log.debug("TTS 请求体 JSON: {}", objectMapper.writeValueAsString(requestBody));
        } catch (Exception ignored) {}

        log.debug("请求 TTS API 的完整地址: {}", apiUrl);
        log.debug("TTS 合成文字: {}", text);
        log.debug("TTS 模型名: {}", ttsModel);
        try {
            // ① 调 TTS API → 返回 JSON（不是二进制音频！）
            ResponseEntity<String> response = restTemplate.postForEntity(
                    apiUrl, request, String.class);
            String responseBody = response.getBody();
            log.debug("TTS 响应 JSON: {}", responseBody);
