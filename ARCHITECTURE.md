...
### v2.12 → v2.13 融合策略之战：RRF、加权，和题目风格的照妖镜

```
问题：真实模型复跑已经把「模型太弱」排除掉，病灶指向融合策略；D-20 预登记的候选是 RRF，
      验收标准也先写死了（文档级 Hit@1 与 MRR 同时超过纯 BM25）。同一轮里补上 D-17 遗留 ① 的
      成对改写题（同一批事实、两种问法），把「题目复用了文档措辞」这个偏置也量出来。

做法：a) HybridFusion 增加 fuseRrf（只吃名次：1/(k+排名)，k=60），与加权融合并列进评测；
      b) questions.tsv 从 26 题扩到 36 题：26 道原文措辞 + 10 道口语改写（成对，同一批事实）；
      c) 评测按 cohort 分组报数，并自动给出 D-20 的验收判定（通过/未通过）。

结论一（RRF 未通过验收，但赢在召回）—— 真实模型口径、36 题：
      文档级 Hit@5：RRF 0.972 > BM25 0.944 > 加权 0.917；片段级 Hit@5：RRF 0.944 > 0.889 > 0.889；
      但排头仍是 BM25 更强（文档 Hit@1 0.667 vs 0.611、MRR 0.774 vs 0.764）。
      → 名次融合换来的是「别漏」（改写组甚至 10/10 全进 top-5），代价是头部精度；按 D-20 的约定，
      它不能顶替默认融合 —— 喂给模型的正是前 4 条，头部变差是净亏。

结论二（改写组才是照妖镜）：口语改写组（10 题）三条通道的文档级 Hit@1 全部掉到 0.40~0.50，
      但**只有真实语义向量反超了 BM25**（0.500 vs 0.400）；离线向量在同组只有 0.200。
      → 这是整个项目里向量通道第一次在排头上赢，而且恰好出现在它被设计来解决的场景（换个说法）；
      同时也说明：现在整条链在这类问题上都太弱，这才是真正的产品瓶颈，而不是融合公式。

结论三（谁也别急着宣布胜利）：36 题里三种策略的差距落在 1~3 题之间（Hit@1 0.667 vs 0.611 就是
      2 题之差），这个样本量不足以宣布某一方「更好」；能可信区分的是**题目风格**（改写组全线掉）。
      → 下一步不是继续调融合，而是「RRF 召回 + 重排」：Reranker 端口与 gte-rerank 集成早就在仓库里
      （默认关闭），验收标准同样先写死（见 D-21）。
```

### v2.13 → v2.14 精排接上：第一次有策略全面赢过纯 BM25

```
问题：D-21 定的路线是「RRF 召回 + 精排」，验收标准也先写死了。但精排这条路一直没跑通过 ——
      一装配就 NoClassDefFoundError: TextReRankParam，根因是 dashscope-sdk-java 太老
      （pom 里写的是 2.16.7，而 rerank 的参数类是 2.22+ 才有的）。

做法：a) dashscope-sdk-java 2.16.7 → 2.22.30；
      b) 把生产里「精排分归一化后与召回分按 0.8 : 0.2 混合」那段抽成 HybridFusion#applyRerankScores，
         **生产与评测共用同一份实现** —— 否则评测量到的和线上跑的是两套逻辑；
      c) 评测新增 -Drag.eval.rerank=dashscope：直接调生产装配（RagConfiguration#ragReranker），
         装完先探一次活；拿不到分数就跳过这两行，并如实写「未跑」，不拿假数据充数。

结论（真实向量 text-embedding-v4 + gte-rerank-v2，36 题）——**精排验收通过**：
      文档级 Hit@1 0.778（BM25 0.667）、MRR 0.851（0.774）、Hit@5 1.000（36 题全部进前 5）；
      片段级 Hit@5 0.944（= RRF 的水平）。三条验收标准全部满足 —— 这是项目里第一次有策略
      在文档级全面超过纯 BM25，而且不是靠调参，是靠加了一层「逐对打分」的模型。

两个副产物比主结论更有意思：
      ① **精排让「用哪种融合」变得不重要**：加权+精排与 RRF+精排的文档级指标完全相同
         （0.778 / 1.000 / 0.851），片段级也只差 1 题。这印证了 D-21 的判断 ——
         融合管召回、精排管排序；精排一接上，融合策略的差异被抹平。
         所以生产默认融合**不必改**（变更面积最小），真正带来提升的是精排这一层。
      ② **口语改写组被精排救活了**：精排后改写组的文档级 Hit@1 从 0.400 升到 0.600，
         反超同组的 BM25（0.400）与纯向量（0.500）—— 首次出现「融合 + 精排」在语义类问题上
         同时赢过两条单通道。
      ③ **片段级 Hit@1 仍是短板**：精排后 0.583~0.611（BM25 0.611），几乎没提升。
...
### 2.21 融合策略对照与题目风格分组（v2.13）

- **两个融合策略并存**：`HybridFusion.fuse`（分数归一化加权，生产默认）与
  `HybridFusion.fuseRrf`（排名融合，1/(k+排名)，k=60）。前者吃分数、后者只吃名次 ——
  名次没有量纲，所以换模型、换语料都不用重新标定；代价是丢掉「第一名领先多少」的信息
- **口径扩展**：题库从 26 题扩到 36 题 —— 26 道原文措辞 + 10 道口语改写（成对：同一批事实、两种问法），
  评测按 `cohort` 分组报数，并自动给出 D-20 的验收判定
- **真实模型口径的成绩**（文档级 Hit@1 / Hit@5 / MRR；片段级见报告）：

| 策略 | 原文措辞组（26 题） | 口语改写组（10 题） | 全量（36 题） |
|---|---|---|---|
| 向量余弦 | 0.731 / 0.962 / 0.822 | **0.500** / 0.900 / **0.653** | 0.667 / 0.944 / 0.775 |
| 关键词 BM25 | **0.769** / 0.962 / **0.844** | 0.400 / 0.900 / 0.592 | **0.667** / 0.944 / 0.774 |
| 加权融合 0.65:0.35 | 0.692 / 0.962 / 0.795 | 0.400 / 0.800 / 0.558 | 0.611 / 0.917 / 0.729 |
| 融合 RRF (k=60) | 0.692 / 0.962 / 0.814 | 0.400 / **1.000** / 0.633 | 0.611 / **0.972** / 0.764 |

- **三条读法**：① RRF 的强项是召回（文档 Hit@5 0.972、片段 0.944，改写组 10/10 进 top-5），
  弱项是头部精度；② 口语改写组里**只有真实语义向量在排头上赢了 BM25**（0.500 vs 0.400），
  离线向量同组只有 0.200 —— 「语义通道有价值，只是离线口径看不见」这句话终于有了证据；
  ③ 三种策略在全量上的差距只有 1~3 题，36 题样本不足以宣布胜负，能可信区分的是题目风格
- **测试**：4 个新单测锁定两种融合的性质（两路都命中优先/只吃名次/同分确定性/单路退化为单通道）；
  评测里 4 条通道 × 2 个 cohort × 2 种向量口径同时出数

### 2.22 精排接入：把「排序」从融合公式里拿出来交给模型（v2.14）

- **为什么要动 SDK 版本**：精排（`QwenScoringModel` → gte-rerank-v2）需要 `dashscope-sdk-java ≥ 2.22`
  （`TextReRankParam`），而 pom 里锁的是 2.16.7 —— 装配时直接 `NoClassDefFoundError`，
  生产代码一直优雅降级成「不精排」，所以这个坑在日志里躺了很久没人发现。已升到 2.22.30
- **一份实现两处用**：精排分与召回分的混合公式抽成 `HybridFusion#applyRerankScores`
  （先按本次候选最大值归一化 → 0.8 × 精排 + 0.2 × 召回 → 倒序），生产 `KnowledgeRetriever`
  与评测共用它。**这是「评测结论能用」的前提**：评测量到的必须就是线上跑的
- **成绩（真实向量 + gte-rerank-v2，36 题，文档级 Hit@1 / Hit@5 / MRR）**：
...
| 加权融合 0.65:0.35 | 0.692 / 0.962 / 0.795 | 0.400 / 0.800 / 0.558 | 0.611 / 0.917 / 0.729 |
| 融合 RRF (k=60) | 0.692 / 0.962 / 0.814 | 0.400 / 1.000 / 0.633 | 0.611 / 0.972 / 0.764 |
| **加权融合 + 精排** | **0.846** / **1.000** / **0.902** | **0.600** / **1.000** / **0.720** | **0.778** / **1.000** / **0.851** |
| **RRF + 精排** | **0.846** / **1.000** / **0.902** | **0.600** / **1.000** / **0.720** | **0.778** / **1.000** / **0.851** |

- **三条读法**：① 精排把文档级三项指标全部拉起（Hit@5 直接拉满 1.000）；
  ② 精排后两种融合的成绩一模一样 —— 说明「融合打架」这个问题的正解不是选边站，而是别让融合独自承担排序；
  ③ 口语改写组里「融合 + 精排」首次同时赢过两条单通道（0.600 vs 0.500 / 0.400）
...
- **已知遗留（写清触发条件，到点能认出来）**：
  ...
  - **③ 语料是公开文档摘录**：33 篇 / 171 片段、五个主题，新闻类只取前若干段。它能证明的是
    ...
    <br/>**追加（v2.13）**：扩到 36 题（+10 道口语改写）后，三种策略在全量上的差距只有 1~3 题 ——
    36 题这个样本量已不足以支撑「某策略更好」的结论。要判定 5 个百分点的差异，题库至少要上百题，
    或者改用配对检验（成对改写题本身就是配对结构，可以直接上 McNemar）。
  ...
  - **⑤ 融合策略只测了一种**：当前只测「归一化加权」，未测 RRF / 重排（Reranker 已在 RAG 链路里，
    ...
    <br/>**已闭环（v2.13）**：RRF 已纳入对照，离线与真实模型两个口径都跑了；结论是 RRF 赢召回、输排头，
    未通过 D-20 的验收标准。下一条路线见 D-21（RRF 召回 + 重排）。
    <br/>**再追加（v2.14）**：精排接上后三条验收标准全部满足（2.22），融合策略的差异被抹平；
    下一个短板是片段级 Hit@1（0.583~0.611），见 D-22 的下一步。
...
### D-21 RRF 的结论：改召回、不改排序，排序交给重排

- **决策**：RRF **不设为生产默认**；它作为「高召回候选策略」保留在代码里并继续参与评测。
  下一条路线是「RRF 召回 + 重排」——Reranker 端口与 gte-rerank 集成早就在仓库里（默认关闭），
  现在把它接上做对照。验收标准先写死：**重排后文档级 Hit@1 与 MRR 同时超过纯 BM25
  （真实模型口径 0.667 / 0.774），且片段级 Hit@5 不低于 RRF 的 0.944**。
- **RRF 没通过验收（D-20 标准）**：真实模型 36 题下，RRF 文档级 Hit@1 0.611 / MRR 0.764，
  BM25 是 0.667 / 0.774 —— 两条线都没超过，所以按预登记的标准不能上默认。
- **但它把一件事量清楚了**：RRF 的召回是四者中最好的 —— 文档级 Hit@5 0.972（BM25 0.944）、
  片段级 0.944（0.889），口语改写组更是 10/10 全进 top-5。也就是说：
  **融合的价值在「别漏」，不在「排得准」**；而排得准直接决定喂给模型的前 4 条质量，
  所以排序得用另一个工具来做（重排），而不是继续在融合公式里找。
- **为什么不把 RRF 设成默认「反正召回更好」**：检索结果的消费方是 prompt 里的 top-4，
  头部精度掉 2 题就是实打实的回答质量损失；把「召回」和「排序」混在一个分数里，
  等于让一个组件同时干两件相反取向的事。
- **为什么不再调融合权重**：离线与真实两个口径的权重扫描（w ∈ [0.20, 0.80]）都没有出现过能赢 BM25 的一档；
  36 题样本下再调就是对着评测集过拟合。
- **为什么改写组的结果比总排行更重要**：它是唯一能区分「词面匹配」与「语义理解」的探针 ——
  真实向量在改写组排头反超 BM25（0.500 vs 0.400），离线向量只有 0.200。
  这条证据说明语义通道不是没用，而是被离线口径与题目风格同时掩盖了；也说明现在的绝对水平太低，
  真正该投入的方向是「改写/扩展查询 + 重排」，而不是在融合公式上做微调。

### D-22 精排：跑通、量化，但默认值留给产品决策

- **决策**：精排链路打通并通过 D-21 验收；`rag.rerank.enabled` 的**默认值仍是 false**，
  由部署方按成本/延迟决定是否开启（配了 key 的场景推荐开）。同时把「精排混合公式」抽成
  生产与评测共用的一份实现。
- **证据（真实向量 + gte-rerank-v2，36 题）**：文档级 Hit@1 0.611 → 0.778、MRR 0.729 → 0.851、
  Hit@5 0.917 → 1.000；口语改写组 Hit@1 0.400 → 0.600（首次同时赢过两条单通道）。
- **为什么已经证明有效，默认还不开**：精排 = 每次检索多一次模型调用（这一轮 36 题 × 2 条口径
  跑了 72 次调用，约 30 秒），这是实打实的延迟与成本；而且它的增益建立在**真实 embedding 模型**
  之上（离线词法向量下候选本身就偏词面，增益没被单独验证过）。把「要不要为质量付这笔钱」留给部署方，
  比替他们默认开启更诚实 —— 配置项与文档已经就绪，一行即可打开。
- **为什么顺手把融合换成 RRF**：精排后两种融合成绩完全相同（0.778 / 1.000 / 0.851），
  换它只增加变更面积。RRF 保留为可切换策略与评测口径（`-Drag.eval.vectorWeight` 与 RRF 都留着）。
- **下一步（片段级才是主战场）**：精排把文档级拉满（Hit@5 1.000），但片段级 Hit@1 仍只有
  0.583~0.611（BM25 0.611）—— 也就是「文档找对了，答案那一片还没排对」。
