...
四种跑法吃同一份数据，`recall` 以进程内暴力检索的 top-k 为精确基准。

| 语料片段 | 向量通道 | 建索引 | p50 | p95 | recall@10 |
|---|---|---|---|---|---|
| 10000 | 进程内暴力（向量） | 647 ms | 3.20 ms | 8.88 ms | 1.000（基准） |
| 10000 | 进程内暴力 + BM25 融合 | 647 ms | 20.00 ms | 33.98 ms | 1.000（基准） |
| 10000 | Qdrant HNSW（向量） | 2403 ms | 2.98 ms | 7.66 ms | 1.000 |
| 10000 | Qdrant + 内存 BM25 融合 | 2403 ms | 22.80 ms | 39.10 ms | 1.000 |
| 100000 | 进程内暴力（向量） | 3631 ms | 34.12 ms | 40.86 ms | 1.000（基准） |
| 100000 | 进程内暴力 + BM25 融合 | 3631 ms | 292.33 ms | 317.59 ms | 1.000（基准） |
| 100000 | Qdrant HNSW（向量） | 14070 ms | 3.20 ms | 6.38 ms | 0.740 |
| 100000 | Qdrant + 内存 BM25 融合 | 14070 ms | 271.70 ms | 408.08 ms | 0.740 |

堆占用（GC 后实测增量）：一万片段 40.7 MB（关键词 + 元信息）→ 50.2 MB（再加向量）；
十万片段 404.5 MB → 503.9 MB —— 切向量库省下的正是后面那一段。

- **数字读出来的三个结论**：① 一万片段时 Qdrant 没有优势（向量通道 2.98ms vs 3.20ms，噪声量级），
  建索引还慢 3.7 倍；② 十万片段时向量通道快 10 倍（3.20ms vs 34.12ms），但**端到端融合只快 7%**
  （271.70ms vs 292.33ms）—— 耗时几乎全被内存里的 BM25 吃掉，换掉向量通道并没有换掉瓶颈；
  ③ HNSW 是近似检索，十万片段 recall@10 = 0.740，而内存暴力扫描是精确解 1.000，这是换向量库真正要付的账
...
- **顺带量出一个真缺陷（如实记录）**：关键词通道是 copy-on-write，**每次增量写入都会整体重编译一次倒排** ——
  实测「追加 1 份文档（10 片段）」在一万片段语料上 55.3ms、十万片段上 417.4ms，随语料线性增长。
  真实规模（几百片段、每次入库一份文档）这份开销可忽略，但批量入库必须走全量重建路径；
  要让增量写入也变成 O(新增)，需要给倒排加增量合并，属于后续优化项（本次未做）
...
| 向量通道 | 默认 in-memory；可切 Qdrant（10 万片段实测 p50 3.20ms / recall@10 0.740，见 2.16） |
...
- **为什么不是「既然做了就默认开启」**：实测不站在这一边（2.16）：一万片段时向量通道 2.98ms 对比
  内存 3.20ms，谈不上优势；十万片段向量通道快 10 倍，但端到端融合只快 7%，瓶颈在内存 BM25；
  同时建索引慢 3.7 倍、recall@10 从 1.000 掉到 0.740。当前真实语料只有几百个片段，
...
- **已知遗留（如实记录）**：① 增量写入仍是「每次全量重编译倒排」（十万片段单次 417ms），
  批量入库要走全量重建路径，增量合并倒排是后续优化项；② HNSW 的 `ef` / `m` 用默认值，未按语料调参；
  ③ 未做副本与运维（数据可从 MySQL 片段表全量回灌，因此不把 Qdrant 当作持久化权威）。
