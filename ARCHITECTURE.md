...
- **已知遗留（写清触发条件，到点能认出来）**：
  ...
  - **② 真实 embedding 模型未复跑**：离线 `local-hashing-v1` 与 BM25 吃同一批 token，向量通道被系统性
    低估，所以生产权重 0.65 : 0.35 目前**没有被验证过**。修法：配上 DashScope key 后只改 `#embed` 一处
    重跑（约 20 分钟 + 文档同步），这才是「混合到底行不行」的判决性实验。
