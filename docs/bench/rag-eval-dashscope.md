...
|---|---|---|---|---|---|---|
| 0.20 | 0.731 | 0.962 | 0.827 | 0.731 | 0.962 | 0.821 |
| 0.35 | 0.731 | 0.962 | 0.827 | 0.731 | 0.962 | 0.827 |
| 0.50 | 0.692 | 0.962 | 0.801 | 0.692 | 0.962 | 0.801 |
| 0.65 | 0.692 | 0.962 | 0.795 | 0.692 | 0.962 | 0.801 |
| 0.80 | 0.692 | 0.962 | 0.795 | 0.462 | 0.962 | 0.670 |

## 切分器对照（同一份语料与题目，走混合通道）

| 切分策略 | 片段数 | 片段均长 | 文档级 Hit@1 | 文档级 Hit@5 | 文档级 MRR | 片段级 Hit@1 | 片段级 Hit@5 | 片段级 MRR |
|---|---|---|---|---|---|---|---|---|
| 自研标题感知切分 | 171 | 548 | 0.692 | 0.962 | 0.795 | 0.692 | 0.962 | 0.801 |
| LangChain4j DocumentSplitters.recursive | 152 | 579 | 0.731 | 0.962 | 0.821 | 0.615 | 0.962 | 0.763 |
...
|---|---|---|---|---|---|---|---|---|
| 框架原生 naive RAG（Ingestor + ContentRetriever，向量单路） | 152 | 0.769 | 0.962 | 0.854 | 0.462 | 0.885 | 0.644 |
| 自研链路（混合召回 + 引用拼装） | 171 | 0.692 | 0.962 | 0.795 | 0.692 | 0.962 | 0.801 |
...
- 文档级 Hit@5：关键词 BM25 0.962、向量 0.962、混合（w=0.65）0.962；当前最强单路是 关键词 BM25，混合落后 0.000（文档级 Hit@5）
- 文档级 Hit@1：混合 0.692、关键词 0.769、向量 0.731；融合换来的是「头部排序更稳」，代价是尾部召回被向量通道稀释。
- 权重扫描：文档级 Hit@5 在 w ∈ [0.20, 0.80] 上最高出现在 w=0.20（0.962），最好的一档 MRR=0.827 —— 真实向量下权重才真正开始起作用（对片段级排序影响明显），
