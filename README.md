...
- [x] 单元测试覆盖核心链路（FC 编排 / 路由 / 历史缓存 / 记忆 / RAG / 记忆检索 / 向量库端口 / 检索评测 / 切分器 / 融合策略，共 139 个）
...
- [ ] RRF 召回 + 重排（接上已有 Reranker 端口与 gte-rerank）；验收标准 = 重排后文档级 Hit@1 与 MRR 同时超过纯 BM25，且片段级 Hit@5 不低于 RRF 的 0.944（D-21）
